...
// When using ngrok (or LAN hostnames), allow those origins so client
// hydration/HMR can work and auth forms don't fall back to full reloads.
...
function extractOriginHost(input: string): string | null {
  if (!input) return null;

  // Wildcard hosts are valid for allowedDevOrigins but aren't valid URL strings.
...
const siteOriginHost = extractOriginHost(process.env.NEXT_PUBLIC_SITE_URL ?? "");
const envAllowedOrigins = (process.env.DEV_ALLOWED_ORIGINS ?? "")
  .split(",")
  .map((value) => extractOriginHost(value.trim()))
  .filter((value): value is string => Boolean(value));

const allowedDevOrigins = Array.from(
  new Set([
    ...DEFAULT_ALLOWED_DEV_ORIGINS,
    ...(siteOriginHost ? [siteOriginHost] : []),
    ...envAllowedOrigins,
  ])
);
