...
export async function POST(request: Request) {
  ...
  // Optional fan-out to n8n (or any webhook consumer). This lets the CRM
  // remain the Meta callback URL while still triggering external workflows.
  // Fire-and-forget by design: n8n outages must not block Meta acks.
  void forwardWebhookToExternal(rawBody, signature)
...
function buildForwardSignature(rawBody: string): string | null {
  const secret = process.env.N8N_WHATSAPP_FORWARD_SECRET
  if (!secret) return null
  return 'sha256=' + crypto.createHmac('sha256', secret).update(rawBody).digest('hex')
}

async function forwardWebhookToExternal(
  rawBody: string,
  metaSignature: string | null,
): Promise<void> {
  const target = process.env.N8N_WHATSAPP_FORWARD_WEBHOOK_URL
  if (!target) return

  try {
    const headers: Record<string, string> = {
      'content-type': 'application/json',
      'x-wacrm-source': 'whatsapp-webhook',
    }
    if (metaSignature) headers['x-meta-signature-256'] = metaSignature

    const forwardSig = buildForwardSignature(rawBody)
    if (forwardSig) headers['x-wacrm-signature-256'] = forwardSig
    ...
    if (!res.ok) {
      console.error('[webhook] external forward failed:', res.status)
    }
  } catch (error) {
    console.error('[webhook] external forward error:', error)
  }
}
